...
public class MartenMigrationManager : IMartenMigrationManager
{
    ...
    public async Task<int> SetupInfrastructure(
        string projectFilePath,
        string databaseName,
        string databaseSchemaName,
        PostgresContainerOptions postgresContainerOptions
    )
    {
        ...
        var migrationFileName =
            $"{timestamp}_{SableCliConstants.InfrastructureSetupMigrationName}.sql";
        var projectDirectory = FileSystemUtilities.ResolveProjectDirectory(projectFilePath);
        var migrationFilePath = Path.Combine(
            projectDirectory,
            "sable",
        ...
        var migration = await template.RenderAsync(
            new
            {
        ...
        var databaseSchemaNameFilePath = Path.Combine(
            projectDirectory,
            "sable",
            databaseName.ToDatabasePathName(),
            "schema.txt"
        ...
        var result = await AddMigration(
            projectFilePath,
            databaseName,
            SableCliConstants.InitialMigrationName,
        ...
        _consoleLogger.LogInfo(
            $"Successfully initialized migration infrastructure in '{Path.Combine(projectDirectory, "sable", databaseName.ToDatabasePathName())}' directory."
        );
        return 0;
    }
    ...
    {
        var readinessProbeStrategy = new ReadinessProbeWaitStrategy(
        ...
        {
            containerBuilder = containerBuilder.WithPortBinding(
    ...
    public async Task<int> AddMigration(
        string projectFilePath,
        string databaseName,
        string migrationName,
        PostgresContainerOptions postgresContainerOptions,
        bool noIdempotenceWrapper = false,
    ...
    )
    {
        ...
        await using var dataSource = NpgsqlDataSource.Create(
            postgresContainerOptions.ConnectionString
        );
        ...
        var patchCommandExecutionResult = await CliWrap
            .Cli.Wrap("dotnet")
            .WithArguments(
                new[]
                {
            ...
            .WithEnvironmentVariables(
                ...
                {
                    [SableConstants.ConnectionStringOverride] =
                        postgresContainerOptions.ConnectionString,
                }
            )
    ...
    public async Task<string> CreateMigrationScript(
        string projectFilePath,
        string databaseName,
        string from = null,
        string to = null
    ...
    {
        var projectDirectory = FileSystemUtilities.ResolveProjectDirectory(projectFilePath);
        var databaseSchemaName = await FileSystemUtilities.ResolveDatabaseSchemaName(
            projectDirectory,
            databaseName
        );
        var scriptDirectoryPath = Path.Combine(
            projectDirectory,
            "sable",
            databaseName.ToDatabasePathName(),
            "migrations"
        ...
        var migrations = Directory
            .EnumerateFiles(scriptDirectoryPath, "*.sql", SearchOption.TopDirectoryOnly)
            .Select(p => new Migration(p, databaseSchemaName))
            .OrderBy(m => m.Timestamp)
            .ToList();
        if (!string.IsNullOrWhiteSpace(from))
        {
            migrations = migrations.SkipWhile(m => m.Id != from && m.Name != from).ToList();
        }
        ...
        // Ensure infrastructure setup migration runs first
        var infrastructureMigration = migrations.FirstOrDefault(m =>
            m.Name == SableCliConstants.InfrastructureSetupMigrationName
        );
        if (infrastructureMigration != null)
        {
            var infrastructureScript = infrastructureMigration.Script;
            var infrastructureTransaction = Template
                .Parse(Templates.TransactionTemplate)
                .Render(new { Script = infrastructureScript }, member => member.Name);
            scriptBuilder.AppendLine($"{Environment.NewLine}{infrastructureTransaction}");

            // Remove infrastructure migration from the list so it's not processed again
            migrations = migrations.Where(m => m.Id != infrastructureMigration.Id).ToList();
        }

        // Process remaining migrations with idempotent wrapping
        var transactions = migrations
            .OrderBy(m => m.Timestamp)
            .Select(m => m.GetTransactionalIdempotentScript())
            .ToList();
        foreach (
            var transactionSegment in transactions.Select(transaction =>
    ...
    public async Task<string> CreateBackfillMigrationScript(
        string projectFilePath,
        string databaseName
    )
    {
        ...
        var migrationInsertionScripts = Directory
            .EnumerateFiles(scriptDirectoryPath, "*.sql", SearchOption.TopDirectoryOnly)
            .Select(p => new Migration(p, databaseSchemaName))
            .OrderBy(m => m.Timestamp)
        ...
        var script = await template.RenderAsync(
            new
            {
